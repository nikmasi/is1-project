...
@NamedQueries({
    ...
    @NamedQuery(name = "Videosnimak.findByKorisnik", query = "SELECT v FROM Videosnimak v WHERE v.vlasnik = :vlasnik"),
    @NamedQuery(name = "Videosnimak.findByKorisnikAndVideo", query = "SELECT v FROM Videosnimak v WHERE v.vlasnik = :vlasnik AND v.idvideosnimak = :idvideosnimak")})
public class Videosnimak implements Serializable {
    ...
    public Videosnimak(Integer idvideosnimak, String naziv, int trajanje, int datum) {
        this.idvideosnimak = idvideosnimak;
        this.naziv = naziv;
